...
export default function TemplatePickerModal({
  open, templates, loading, loadingNicho, onSelect, onSkip,
}: Props) {
  if (!open) return null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
              ...
              {TEMPLATES.map(tmpl => {
                ...
                return (
                  <TemplateCard
                    ...
                    onUse={() => { if (apiTmpl) onSelect(apiTmpl) }}
                  />
                )
              })}
            </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-gray-100 shrink-0 flex items-center justify-end">
          <button
            onClick={onSkip}
